"""Verify the vendored company-role vocabulary, schemas, and fixtures match their recorded digests."""
...
SOURCE_RECORDS = (
    ROOT / "src" / "common" / "company_role_vocabulary" / "source.json",
    ROOT / "tests" / "fixtures" / "company-roles" / "source.json",
)


def check_source_record(source_record: Path) -> None:
    """Verify every file listed in one source.json against its recorded digest."""
    assert source_record.is_file(), f"company-role vocabulary source record is missing: {source_record}"
    ...
    for entry in source["files"]:
        ...
        assert vendored_file.is_file(), f"vendored company-role file is missing: {vendored_file}"

        actual_digest = sha256(vendored_file.read_bytes()).hexdigest()
        assert actual_digest == entry["sha256"], (
            f"company-role vocabulary digest drift for {entry['vendored_as']}: expected {entry['sha256']} "
        ...
        print(f"company-role vocabulary digest verified: {actual_digest} ({entry['vendored_as']})")


def main() -> None:
    """Check every recorded company-role vocabulary and fixture source record."""
