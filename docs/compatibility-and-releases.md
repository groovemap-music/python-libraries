...
line is Python 3.14, and CI pins that line to Python 3.14.7 through `.mise.toml`.
...
2. Run `just audit` and `just release-dry-run` against the reviewed source. The dry-run recipe
   depends on the complete `check` graph, then records checksums, a CycloneDX SBOM, third-party
   notices, and commit-bound local provenance for its validated build in ignored `dist/` output.
